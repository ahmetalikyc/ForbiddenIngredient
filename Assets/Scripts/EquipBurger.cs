...
public class EquipBurger : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        if (Service == null)
        {
            Service = FindFirstObjectByType<Service>();
        }
    ...
    void Equip()
    {
        ...
        transform.SetParent(BurgerParent.transform);
        transform.SetPositionAndRotation(BurgerParent.position, BurgerParent.rotation);
    }
    void Drop()
    {
        isHeldByMe = false;
