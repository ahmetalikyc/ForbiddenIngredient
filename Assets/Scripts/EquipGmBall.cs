...
public class EquipGmBall : MonoBehaviour
{
    ...
    private MeshCollider groundMeatCollider;

    private void Awake()
    {
        rb = groundMeat.GetComponent<Rigidbody>();
        groundMeatCollider = groundMeat.GetComponent<MeshCollider>();
    }
    ...
    void OnRayC()
    {
        ...
        {
            if (hit.collider.CompareTag("groundMeat") && CarryManager.HoldingItem == false)
            {
